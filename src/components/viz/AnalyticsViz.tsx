/**
 * AnalyticsViz — editorial charts that "form" slowly.
 * Line chart + bar chart in a monograph grid.
 */
  ...
  return (
    <div className="viz" data-viz="analytics">
      <div className="viz__head">
        <span className="viz__head-num">FIG. 10</span>
        <span className="viz__head-rule" />
        <span className="viz__head-label">ANALYTICS / PATTERNS</span>
      </div>
        ...
        <div className="analytics__grid">
          <div className="analytics__panel analytics__line">
            ...
            <svg viewBox={`0 0 ${W} ${H}`} className="analytics__svg" preserveAspectRatio="none">
              ...
              <path d={linePath} fill="none" stroke="#ff5b1f" strokeWidth="0.7" className="an-line" />
              {linePts.map((p, i) => (
                <circle
                  key={i}
                  cx={i * step}
            ...
            <div className="analytics__bartrack">
              {bars.map((b, i) => (
                <div
                  ...
                  key={i}
                  style={{ height: `${b}%`, animationDelay: `${i * 0.08}s` }}
                />
              ))}
            </div>
          </div>
      ...
      <style>{`
        ...
        .analytics__svg { width: 100%; height: 110px; display: block; }
        .an-line {
